(defun spass-get(account)
  ...
  (setq password (replace-regexp-in-string "[\r\n]*" "" (shell-command-to-string (concat "spass --print --get " account))))
  (kill-new password)
  (message "Done")
)
(defun spass-update(account)
  (interactive "sPlease specify the account: ")
  (setq password (replace-regexp-in-string "[\r\n]*" "" (shell-command-to-string (concat "spass --update " account))))
  (kill-new password)
  (message "Done")
)
(defun spass-update-simple(account)
  (interactive "sPlease specify the account: ")
  (setq password (replace-regexp-in-string "[\r\n]*" "" (shell-command-to-string (concat "spass --simple --update " account))))
  (kill-new password)
  (message "Done")
)
